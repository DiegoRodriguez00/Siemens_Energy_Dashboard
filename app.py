...
st.markdown('<h1 class="main hear">💡 Dashboard Siemens Energy 2025</h1>', unsafe_allow_html=True)

#Sistemas de filtros profesional:

col1,col2,col3 = st.columns(3)
with col1:
    periodo = st.selectbox ("📅 Periodode analisis:", ["Ultimos 30 dias","Ultimo trimestre", "Ultimo año", "Todo el perido"])
with col2:
    categoria = st.selectbox ("🎯 Categoria:", ["General", "Ventas", "Marketing","Producto","Finanzas"])
with col3:
    comparacion = st.selectbox("📊 Comparar con:", ["Periodo anterior", "Mismo periodo año pasado", "Promedio historico"])
...
## Graficos
st.markdown ("## 📊 Analisis de Tendencias")
...
with col1:
    ...
    fig.update_layout(title="💰 Evolucion de Ingresos", height=400, template="plotly_white")
...
with col2:
    ...
    funnel.update_layout(title="🎯 Funnel de Conversion", height=400, template="plotly_white")
    st.plotly_chart(funnel, use_container_width=True)


# Cuadro Analisis Geograficos
st.markdown("## 🌏 Analisis Geografico")
paises = ['Mexico','Colombia', 'Chile', 'Argentina','Peru', 'España']
ventas_paises = np.random.uniform(10000,100000, len(paises))
mapa = px.bar(x=paises,y=ventas_paises, color=ventas_paises, color_continuous_scale = 'Viridis',title="💎 Ventas por Region")
...
if df['ingresos_diarios'].tail(7).mean() < df['ingresos_diarios'].head(-7).mean():
    alertas.append({'tipo':'⚠️ Advertencia' ,'mensaje':'Ingresos por debajo del promedio en ultimos 7 dias','color':'orange'})
...
if df['usuarios_activos'].tail(1).iloc[0] > df['usuarios_activos'].quantile(0.9):
    alertas.append({'tipo':'💪  Exito' ,'mensaje':'Usuariosactivos en top 10% historico','color':'green'})
